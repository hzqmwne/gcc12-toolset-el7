...
/opt/gcc12-toolset/root/usr/
    ...
    lib64/：仅 full 可见的完整 64 位 GCC 动态/静态运行库
    lib/：仅 full 可见的完整 32 位 GCC 动态/静态运行库

/opt/gcc12-toolset/profiles/compat/
    DTS patched multilib headers
    64/32 位 libstdc++_nonshared.a
    分别指向 /usr/lib64 和 /usr/lib 系统 libstdc++.so.6 的 linker script
    g++/c++ wrapper
```
...
/opt/gcc12-toolset/root/usr/lib/libstdc++.so.6.0.30  # -m32
```

支持 ABI 0 与 ABI 1，默认 ABI 1，符号上限为 `GLIBCXX_3.4.30`。
ASan、UBSan、TSan、LSan 和 `std::stacktrace` 也使用 `full` profile 的私有
运行库或静态 backtrace archive。
...
/usr/lib/libstdc++.so.6  # -m32
+ /opt/gcc12-toolset/profiles/compat/.../32/libstdc++_nonshared.a
```
...
- 不暴露私有 sanitizer 运行库；sanitizer 构建应使用 `full` profile；
