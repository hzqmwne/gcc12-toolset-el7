...
Graphite 使用同一官方 spec 选择的 GCC upstream ISL 0.24 归档；ISL 在 GCC
构建目录内私有编译，不依赖 EL7 仓库，也不安装到系统路径。

## 2. 源码校验

CentOS 历史归档使用 lookaside SHA-1；新增依赖和补丁使用固定 SHA-256：
...
isl-0.24.tar.bz2
  SHA-256 fcf78dd9656c10eb8cf9fbd5f59a0b6b01386205fe1934b3b287a0a1898145c0
...
手动准备离线源码时，把 GCC、ISL、binutils 和 compat patch 四个文件放到
`cache/`，脚本仍会逐一验证固定哈希。
...
4. 构建并安装私有 binutils；
5. 私有构建锁定的 ISL 0.24；
6. profiled bootstrap 完整 GCC 12；
7. 对源码副本应用 DTS 兼容补丁，精简构建 compat headers 与 nonshared archive；
8. 构建所有二进制 RPM 和 SRPM；
9. 导出 RPM SHA-256 清单。
