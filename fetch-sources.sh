...
fetch_sha256 "$ISL_SOURCE" "$ISL_SHA256" "$ISL_URL" "$ISL_FALLBACK_URL"
fetch_one "$BINUTILS_SOURCE" "$BINUTILS_SHA1" \
    "$BINUTILS_URL" "$BINUTILS_FALLBACK_URL"
fetch_sha256 "$MAKE_SOURCE" "$MAKE_SHA256" "$MAKE_URL" "$MAKE_FALLBACK_URL"
...
(
    cd "$DEST"
    sha256sum "$GCC_SOURCE" "$ISL_SOURCE" "$BINUTILS_SOURCE" "$MAKE_SOURCE" \
        "$LIBSTDCXX_COMPAT_PATCH" \
